  ...
  return {
    ...
    after: (tableElement) => {
      if (tableElement) {
        ...
        const newElement = tableElement.cloneNode() as HTMLTableElement;
        const colGroup = document.createElement('colgroup');
        const tBody = document.createElement('tbody');
        
        // Move children to tbody
        tBody.append(...tableElement.children);
        
        // Find first row node using Lexical children
...
export function convertTableElement(_domNode: Node): DOMConversionOutput {
  ...
  console.log("stp2",$createTableNode())
